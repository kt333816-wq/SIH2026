...
router.post('/', async (req, res) => {
    ...
    try {
        // latitude/longitude are NUMERIC columns; ST_MakePoint expects double
        // precision. $4/$5 are each used twice below, so Postgres needs an
        // explicit cast on every use or it can't settle on one type for the
        // placeholder (that's what caused the "inconsistent types deduced"
        // crash previously).
        ...
        res.status(500).json({ error: 'Could not create the listing - please try again' });
    }
});
...
router.get('/:id/status', async (req, res) => {
    try {
        ...
        console.error('Failed to fetch listing status:', err.message);
        res.status(500).json({ error: 'Could not fetch listing status' });
    }
});
...
router.get('/:id/tracking', async (req, res) => {
    try {
        const { rows } = await pool.query(
            `SELECT fl.donor_id, fl.matched_receiver_id, fl.latitude AS donor_lat, fl.longitude AS donor_lon,
                    ...
                    CASE WHEN u.live_latitude IS NOT NULL
                         THEN ST_Distance(fl.location, ST_SetSRID(ST_MakePoint(u.live_longitude::double precision, u.live_latitude::double precision), 4326)::geography)
        ...
        console.error('Failed to fetch tracking info:', err.message);
...
router.post('/:id/verify-pickup', async (req, res) => {
    ...
    try {
        ...
        console.error('Failed to verify pickup:', err.message);
        res.status(500).json({ error: 'Could not verify pickup - please try again' });
    }
});
